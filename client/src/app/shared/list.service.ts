...
import { GenericResponse, SortableItem } from './interfaces';
...
export class ListService {
  ...
  get(): Observable<SortableItem[]> {
    return this.http.get(`${this.ENPOINT}${this.LIST_ROUTE}/`).pipe(
      map((res) => res.json()),
      map((res) => {
  ...
  create(list): Observable<Array<SortableItem>> {
    ...
    return this.http.post(`${this.ENPOINT}${this.LIST_ROUTE}/`, list).pipe(
      map((res) => res.json()),
      map((newList) => {
   ...
   */
  edit(list: SortableItem): Observable<GenericResponse> {
    return this.http.put(`${this.ENPOINT}${this.LIST_ROUTE}/${list._id}`, list).pipe(
      map((res) => res.json()),
   ...
   * @returns Observable<GenericResponse>
   */
  remove(list: SortableItem): Observable<GenericResponse> {
    return this.http.delete(`${this.ENPOINT}${this.LIST_ROUTE}/${list._id}`).pipe(
      map((res) => res.json()),
      catchError((err) => Observable.throw(err.json())));
  }
